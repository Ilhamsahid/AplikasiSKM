...
umurInput.addEventListener("input", () => {
	const value = umurInput.value;

	// KALAU MASIH KOSONG → DIAM
	if (value === "") {
		console.log("KOSOSNG");
		umurError.classList.add("hidden");
		umurInput.classList.remove("border-red-500", "border-green-500");
		umurInput.classList.add("border-gray-300");
		return;
	}
	...
	if (umur < 1) {
		...
		umurInput.classList.remove("border-gray-300", "border-green-500");
	} else {
		umurError.classList.add("hidden");
		umurInput.classList.remove("border-red-500");
	}
});
